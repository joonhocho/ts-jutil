...
describe('Value', () => {
  test('basic', () => {
    ...
    const vs1: Array<[number, number | undefined]> = [];
    const off1 = value.on((v, pv) => vs1.push([v, pv]));
    value.set(1);

    const vs2: Array<[number, number | undefined]> = [];
    ...
    const vs3: Array<[number, number | undefined]> = [];
    ...
    expect(vs1).toEqual([[1, undefined], [2, 1]]);
    expect(vs2).toEqual([[2, 1], [3, 2], [4, 3]]);
