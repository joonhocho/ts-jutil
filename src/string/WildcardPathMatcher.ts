...
export class WildcardPathMatcher {
  ...
  public tooWildCount: number;
  public beforeTooWild: string[];
  public afterTooWild: string[];
  public tooWild: typeof wild01 | typeof wild0p | typeof wild1p | null = null;
  ...
  constructor(pattern: string[]) {
    ...
    this.tooWildCount = tooWildCount;
    this.beforeTooWild = beforeTooWild;
    this.afterTooWild = afterTooWild;
  }

  public match(match: string[]): boolean {
    const { length } = match;
    if (length >= this.minLength && length <= this.maxLength) {
      const { beforeTooWild } = this;
